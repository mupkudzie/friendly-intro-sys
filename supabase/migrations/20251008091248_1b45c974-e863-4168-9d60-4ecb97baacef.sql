-- Fix ambiguous column reference in handle_task_completion function
...
BEGIN
  ...
  IF NEW.status = 'approved' AND OLD.status != 'approved' THEN
    ...
    VALUES (NEW.assigned_to, week_start_date, 1, 8)
    ...
    -- Check total hours for program completion (fixed ambiguous column reference)
    SELECT COALESCE(SUM(time_logs.total_hours), 0) INTO total_hours
